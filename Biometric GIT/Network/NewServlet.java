    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(status){
                ...
                PreparedStatement pstmt3=con.prepareStatement("select * from file_storage where block1 LIKE '%"+block1+"%'  or  block2 LIKE '%"+block2+"%' or  block3 LIKE '%"+block3+"%'");
                ResultSet result=pstmt3.executeQuery();
                if(result.next())
                {
                      response.sendRedirect("upload.jsp?msgerror=File_upload_to_cloud");
                }else{
